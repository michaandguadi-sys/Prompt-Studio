...
import {
  Download, Maximize2, Minimize2, PanelLeftClose, PanelLeftOpen,
  PanelRightClose, PanelRightOpen, Keyboard, Film,
} from "lucide-react";
import { StyleEditor } from "@/components/StyleEditor/StyleEditor";
import { SceneBuilderSwitch } from "@/components/SceneBuilderSwitch/SceneBuilderSwitch";
import { Timeline } from "@/components/Timeline/Timeline";
import { ExportDialog } from "@/components/ExportDialog/ExportDialog";
import { PresetBar } from "@/components/PresetBar/PresetBar";

const EditorPreviewPanel = dynamic(
  () => import("@/components/EditorPreviewPanel/EditorPreviewPanel").then((m) => m.EditorPreviewPanel),
...
export default function SequenceEditorPage() {
  // NOTE: unlike the per-kind quick-entry pages, the editor does NOT reset the
  // project on mount — it edits whatever sequence is already in the store.
  const activeKind = useStudio((s) => s.spec.kind);
  const activeName = useStudio((s) => s.spec.name);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        {/* Left: builder for the active scene */}
        {showLeft && (
          <aside className="flex w-[340px] shrink-0 flex-col border-r border-line/60 bg-white">
            <header className="flex items-center justify-between border-b border-line/60 px-5 py-3.5 bg-white/80">
              <div className="flex items-center gap-2.5">
                <div className="flex h-7 w-7 items-center justify-center rounded-md bg-amber/10 border border-amber/20 text-amber">
                  <Film size={14} />
                </div>
                <div>
                  <h1 className="text-xs font-semibold text-graphite leading-none">Sequence Editor</h1>
                  <p className="text-[9px] text-graphite/45 mt-0.5 uppercase tracking-wider">
                    {KIND_LABELS[activeKind] ?? activeKind} · {activeName}
                  </p>
                </div>
              </div>
            </header>
        ...
        {/* Center: full-sequence preview */}
        <div className="flex-1 min-w-0 overflow-hidden">
          <EditorPreviewPanel />
        </div>

        {/* Right: code for the active scene */}
        {showRight && (
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={k} className="flex items-center gap-3">
                  <kbd className="rounded-md bg-graphite/[0.05] border border-line px-2.5 py-1 font-mono text-[10px] text-amber min-w-[100px] text-center">{k}</kbd>
...
const ToggleBtn: React.FC<{
  ...
  <button onClick={onClick} title={title}
    className={["rounded-md p-1.5 transition-all duration-150",
      active ? "bg-amber/15 text-amber" : "text-graphite/50 hover:text-graphite hover:bg-graphite/[0.06]",
    ].join(" ")}>
    {icon}
  </button>
);
