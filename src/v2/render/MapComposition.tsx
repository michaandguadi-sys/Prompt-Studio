      ...
      >
        ...
        {[...comp.layers].reverse().map((l, ri) => {
          ...
          if (l.type === "highlight" && l.geojson)
            return <HighlightSource key={`${l.id}-${ri}`} layer={l} frame={frame} fps={fps} totalFrames={totalFrames} />;
...
const HighlightSource: React.FC<LV<HighlightLayer>> = ({ layer: l, frame, fps, totalFrames }) => {
  ...
  return (
    ...
    <>
      <Source id={`${l.id}-glowS`} type="geojson" data={data}>
        <MapLayer id={`${l.id}-glow`} type="line" paint={{ "line-color": l.glowColor, "line-width": l.glowWidth, "line-blur": 10, ...glowPaint }} layout={{ "line-cap": "round", "line-join": "round" }} />
      </Source>
