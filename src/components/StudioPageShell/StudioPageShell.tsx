...
import {
  ...
  PanelLeftClose, PanelLeftOpen, PanelRightClose, PanelRightOpen, Keyboard,
} from "lucide-react";
...
interface StudioPageShellProps {
  ...
  iconColor: string;
  onReset: () => void;
  children: React.ReactNode; // builder component(s)
}

export function StudioPageShell({ title, icon, iconColor, onReset, children }: StudioPageShellProps) {
        ...
        <aside className="flex w-[340px] shrink-0 flex-col border-r border-line/60 bg-white">
          <header className="flex items-center justify-between border-b border-line/60 px-5 py-3.5 bg-white/80">
            <div className="flex items-center gap-2.5">
              <div className={`flex h-7 w-7 items-center justify-center rounded-md border ${iconColor}`}>
                {icon}
              </div>
              <div>
                <h1 className="text-xs font-semibold text-graphite leading-none">{title}</h1>
